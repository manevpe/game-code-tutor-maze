...
BlocklyStorage.makeRequest =
  ...
  xhr.onload = function() {
    if (this.status === 200) {
      ...
      BlocklyStorage.alert_("There was a problem with the request." +
...
BlocklyStorage.handleLinkResponse_ = function() {
  ...
  BlocklyStorage.alert_("Share your blocks with this link:\n\n%1".replace('%1',
...
BlocklyStorage.handleRetrieveXmlResponse_ = function() {
  const data = this.responseText.trim();
  if (!data.length) {
    BlocklyStorage.alert_("Sorry, '%1' doesn't correspond with any saved program."
